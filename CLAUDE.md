...
- To decode any web/mobile order-form field format, create the order in the app then GET `/autoorder/api/v1` and read the saved fields back. That is how `trailing_stop_type=1` (Last Price) and the raw-percent `ratio_*` / `*_percentage` formats were pinned, no proxy needed. Sell conditions wired: `tp`/`sl` (`ratio_profit`/`ratio_loss`), `trail` (`after_gaining_percentage` + `sell_if_drop_percentage` + `trailing_stop_type` 1). Still not wired: buy Drop-by-% and Profit/Loss in Rp
...
- `order.ts` (`deno task order <cmd>`): place/manage Growin orders. `buy`/`sell <sym> <lot> <cond> <exec>` = auto-order (conditional), where `<cond>` is `le=/ge=<price>`, or (sell-only) `tp=<pct>` / `sl=<pct>` / `trail=<gain%>,<drop%>`, and `<exec>` is `at=<price>` or `tick=<n>` (shorthand: a bare price). `stop`/`resume`/`cancel` manage it. `dbuy`/`dsell` = direct order (instant fill over WS), `dwithdraw`/`damend` pull or reprice a resting one. Nothing fires without a command
