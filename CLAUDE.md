...
- Direct order WS shares Growin's single session with the app. With the app open the order frame still lands but the ack often never returns. `placeDirectOrder` recovers by finding the just-placed order in the order-list (by symbol/side/price) and rebuilding the ack, so it only throws when the order truly did not land. Still prefer closing the app first
