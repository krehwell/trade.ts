...
- Edit in place = `PUT /autoorder/api/v1` with the full create payload + `auto_order_uuid` set. The API 422s while the order is running and rejects unknown fields, so `updateAutoOrder` pauses, PUTs a clean payload (row values merged with the edits), then plays again
...
- To decode any web/mobile order-form field format, create the order in the app then GET `/autoorder/api/v1` and read the saved fields back. That is how `trailing_stop_type=1` (Last Price) and the raw-percent `ratio_*` / `*_percentage` formats were pinned, no proxy needed. Conditions wired: price `le`/`ge`, buy `drop` (`drop_percentage` + `drop_price_type` 1 = From Highest), sell `tp`/`sl` (`ratio_profit`/`ratio_loss`, percent), `tpRp`/`slRp` (`total_profit`/`total_loss`, rupiah), and `trail` (`after_gaining_percentage` + `sell_if_drop_percentage` + `trailing_stop_type` 1). Multiple conditions on one order combine with OR. The whole auto-order UI condition set is now wired
- Direct order WS shares Growin's single session with the app. With the app open the order frame still lands but the ack often never returns. All three actions recover from a lost ack via the order-list: place finds the just-placed order (by symbol/side/price), withdraw/amend confirm the target flipped to CANCELED/AMENDED. They only throw when the action truly did not apply. Still prefer closing the app first
- Amend/withdraw need `internalId`+`sequence`. They come from the place ack, but the order-list REST also carries them: `internalId` = `user_order_id`, `sequence` = `market_order_id`. So `resolveOrderRef(id)` looks them up (accepts the `GW...@...` marketId or the plain transaction id the account view shows, walks all pages) and `dwithdraw`/`damend` work on ANY resting order, including app-placed ones (rejected orders show 0, they have no real ids)
